...
def mouseMove(coordKeyPrime, coordKeySecond, startRandInt, endRandInt, endPoint):
    ...
    endPrimeLoc = endPoint[coordKeyPrime] - getNumber(endPoint[coordKeyPrime], start, randIntRange)
    if (loc1[coordKeyPrime] > endPoint[coordKeyPrime]):
        endPrimeLoc = endPoint[coordKeyPrime] - getNumber(start, endPoint[coordKeyPrime], randIntRange)
    ...
    if (loc1[coordKeySecond] == endPoint[coordKeySecond]):
        endPrimeLoc = endPrimeLoc - random.randint(randIntRange[0], randIntRange[1])
    ...
    for x in range(start, endPrimeLoc, random.randint(startRandInt, endRandInt)):
        ...
        if (ydiff == 'add' and y <= endPoint[coordKeySecond]):
            y = y + random.randint(round(yRandInt/2), yRandInt)
        elif (ydiff == 'add' and y >= endPoint[coordKeySecond]):
            y = y - random.randint(round(yRandInt/2), yRandInt)
            
        if (ydiff == 'subtract' and y >= endPoint[coordKeySecond]):
            y = y - random.randint(round(yRandInt/2), yRandInt)
        elif (ydiff == 'subtract' and y <= endPoint[coordKeySecond]):
            y = y + random.randint(round(yRandInt/2), yRandInt)
            
            
        if (xdiff == 'add' and x <= endPoint[coordKeySecond]):
            x = x + random.randint(round(yRandInt/2), yRandInt)
        elif (xdiff == 'add' and x >= endPoint[coordKeySecond]):
            x = x + random.randint(round(yRandInt/2), yRandInt)
            
        if (xdiff == 'subtract' and x >= endPoint[coordKeySecond]):
            x = x - random.randint(round(yRandInt/2), yRandInt)
        elif (xdiff == 'subtract' and x <= endPoint[coordKeySecond]):
            x = x + random.randint(round(yRandInt/2), yRandInt)
